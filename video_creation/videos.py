...
from utils.CONSTANTS import FAMILY_GUY_URL
...
def download_familyguy():
    """ Downloads the Family Guy footage needed for the video """
    Path("./assets/background/").mkdir(parents=True, exist_ok=True)
    if Path("assets/background/family.mp4").is_file():
        return
    print("We are downloading the Family Guy Clips!")
    ...
    YouTube(URL, on_progress_callback=on_progress).streams.filter(res="1080p").first().download(
        "assets/background", filename="family.mp4"
    )
...
def chop_background_video():
    ...
    """
    background = VideoFileClip(f"assets/background/family.mp4")
    start_time, end_time = get_start_and_end_times(15, background.duration)
    try:
        ffmpeg_extract_subclip(
            f"assets/background/family.mp4",
            start_time,
            end_time,
            targetname=f"assets/temp/family.mp4",
        )
    except (OSError, IOError):  # ffmpeg issue see #348
        print("FFMPEG issue. Trying again...")
        with VideoFileClip(f"assets/background/family.mp4") as video:
            new = video.subclip(start_time, end_time)
            new.write_videofile(f"assets/temp/family.mp4")
